# PatchSet3D — Architecture Notation Reference

Working reference for keeping notation consistent across `3_method.tex`, the TikZ
figures (`imgs/method/arch_patchset.tex`), and the actual implementation
(`src/models/patchset3d.py`, `src/models/pfn_seg_2d.py`,
`experiments/3d/cascade.py`). Symbols follow `3_method.tex` wherever it already
defines one; new symbols are added only where the paper draft is silent. Values
tagged **[ref-config]** are the concrete numbers of the checkpoint currently used
for every eval-table row (`experiment=92_multisource_synth`, model `patchset3d`,
see `docs/datasets/eval_expansion_status.md`) — they instantiate the notation,
they are not part of the notation itself.

Where the implementation has simplified or diverged from what `3_method.tex`
currently states, this doc says so explicitly (flagged **[divergence]**) rather
than silently picking one — those are exactly the spots that need a decision
before the method section's `\TODO`s can be closed.
