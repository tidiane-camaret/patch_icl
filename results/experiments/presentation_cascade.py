...
def _():
    ...
    from totalseg_geometry_extract import geometry_for_pairs, shape_families, TOTALSEG_ROOT

    pd.set_option("display.width", 240)
    pd.set_option("display.float_format", lambda v: f"{v:.3f}")

    PROJECT = "tidiane-camaret-ndir-universit-tsklinikum-freiburg/patchset_train"
    # 88_cascade = CT-only [6,3,1.5] cascade (f4exnxov, killed ep54, val/dice 0.577).
    # 89_multisource_cascade = same cascade + CT/MRI cohort provider (0c47sbeu, crashed
    # ep52, val/dice 0.469) -- the natural pair per 89's own docstring ("88_cascade +
    # dataset=multisource_ct_mri"). Swap in other run ids as needed.
    RUNS = {"patchset_84_cascade": "v48ucp2c", "medverse_91_multisource": "dm8f4jor"}
    ...
    # TotalSegmentator (CT) and totalsegmri (MRI) subjects share the SAME "sNNNN" id
    # namespace (e.g. s0355 exists, as a DIFFERENT scan, in both roots) -- geometry MUST be
    # looked up per target-modality root, keyed on (subject, tgt_mod, class), never bare
    # (subject, class), or a cascade/multisource run's MRI-target rows silently pick up the
    # wrong subject's CT mask geometry.
    TOTALSEGMRI_ROOT = Path(
        "/nfs/data/nii/data1/Analysis/camaret___in_context_segmentation/"
        "ANALYSIS_20251122/data/totalsegmri")
    GEOM_ROOT_BY_MOD = {"ct": TOTALSEG_ROOT, "mri": TOTALSEGMRI_ROOT}

    def _load(_id):
        """Load one run's cached samples table + meta, refetching from W&B on cache miss
        or when CHECK_FRESH and W&B holds a newer run_table version than the cache."""
        ...
        if _samp_c.exists() and _meta_c.exists():
            ...
            if not CHECK_FRESH:
                return pd.read_csv(_samp_c), _m
            import wandb
            _run = wandb.Api().run(f"{PROJECT}/{_id}")
            if latest_table_version(_run) <= _m.get("table_version", -1):
                return pd.read_csv(_samp_c), _m       # cache matches latest → no download
            print(f"  [{_id}] newer table on W&B (cached v{_m.get('table_version', -1)}) → refetching")
        if _run is None:
            import wandb
            _run = wandb.Api().run(f"{PROJECT}/{_id}")
        _s, _ver = get_latest_table(_run, table_key="val/samples.table.json", return_version=True)
    ...
    # regime/modality from the source-adaptive `detail` string (multisource cohort runs:
    # "<regime> <tgt_mod><-<ctx_mod>[ [fb]]"; single-source runs log detail="" -> ct/ct/ct).
    _det = S["detail"].fillna("").str.extract(r"^(?P<regime>\S+) (?P<tgt_mod>\w+)<-(?P<ctx_mod>\w+)")
    S["regime"] = _det["regime"].fillna("ct")
    S["tgt_mod"] = _det["tgt_mod"].fillna("ct")
    S["ctx_mod"] = _det["ctx_mod"].fillna("ct")
    print("regime × run (blank detail -> single-source 'ct'):")
    print(S.groupby(["run", "regime"]).size().unstack("regime", fill_value=0).to_string())

    # shared morphology families + per-sample geometry from ALL evaluated (subject,class) real
    # masks. totalseg (CT) and totalsegmri (MRI) subjects reuse the SAME sNNNN ids, so geometry
    # is looked up per tgt_mod root and merged on (subject, tgt_mod, class) -- NOT bare
    # (subject, class), which would silently mix up cross-modality subject pairs.
    _cache = _ARTIFACTS / f"21_cmp_{'_'.join(RUNS.values())}_geometry.csv"
    _pairs = S[["subject", "tgt_mod", "class"]].drop_duplicates()
    if _cache.exists():
        GEOM = pd.read_csv(_cache)
        _need = set(_pairs.itertuples(index=False, name=None))
        _have = set(zip(GEOM.subject, GEOM.tgt_mod, GEOM["class"])) if len(GEOM) else set()
        if not (_need <= _have):
            GEOM = None
    else:
        GEOM = None
    if GEOM is None:
        _parts = []
        for _mod, _grp in _pairs.groupby("tgt_mod"):
            _root = GEOM_ROOT_BY_MOD.get(_mod, TOTALSEG_ROOT)
            _g = geometry_for_pairs(_grp[["subject", "class"]], root=_root)
            _g["tgt_mod"] = _mod
            _parts.append(_g)
        GEOM = pd.concat(_parts, ignore_index=True)
        _cache.parent.mkdir(parents=True, exist_ok=True)
        GEOM.to_csv(_cache, index=False)
    SHAPE, SHAPE_ORDER = shape_families(GEOM, k=N_SHAPE)
    S = S.merge(GEOM, on=["subject", "tgt_mod", "class"], how="left")
    S["shape"] = S["class"].map(SHAPE).fillna("other")
    if (S["shape"] == "other").any():
        SHAPE_ORDER = SHAPE_ORDER + ["other"]
    # cascade eval never logs tgt_size (single-level-only field, evaluate._occupancy_stats) --
    # backfill from GEOM's 128³ whole-body `volume` so cells 1/1b's "med tgt size" readouts
    # aren't silently all-NaN on a cascade run. Coarser (128³ vs the sample's own crop grid)
    # but same relative ordering across classes.
    S["tgt_size"] = S["tgt_size"].fillna(S["volume"])
...
def _(RUN_NAMES, S, SHAPE_ORDER, pd, plt):
    ...
    # A single-source run (e.g. 88_cascade) only ever sees regime="ct"; a multisource cohort
    # run (89) pools ct/mri/cross tasks into the SAME macro/micro figure above, so that headline
    # number is not apples-to-apples with a CT-only run. Break it out by regime so a cascade vs
    # multisource-cascade comparison isn't silently confounded by task mix.
    if (S["regime"] != "ct").any():
        _by_regime = S.groupby(["run", "regime"]).dice.mean().unstack("regime").reindex(index=RUN_NAMES)
        print("\nmicro dice by regime × run ('ct' is the only slice a CT-only run ever sees):")
        print(_by_regime.to_string())
...
def _(RUN_NAMES, S, pd, plt):
    ...
    for _k, (_f, _logx) in enumerate(_drivers):
        _ax = _axes[_k]
        # cascade eval (experiments/3d/cascade.py's stitched-metrics case dict) never
        # populates tgt_size/tgt_occ/ctx_size/ctx_occ (those are single-level-only, set by
        # evaluate._occupancy_stats) -- so a cascade run's column is ALL-NaN, and after the
        # >0 filter a log-scale driver has NOTHING to plot; matplotlib then errors trying to
        # tick-locate an empty/non-positive axis. Skip the panel instead of crashing.
        if _f not in S.columns or not (S[_f].dropna() > 0).any():
