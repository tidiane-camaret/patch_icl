...
from feature_sim.metrics import (
    l2norm, auroc, average_precision, soft_auroc,
...
def test_label_transfer_dice_rewards_aligned_features():
    # transfer_dice replaces the old min-max soft_dice as the segmentation-quality
    # proxy: separable features (FG->FG, BG->BG under 1-NN) should score near-perfect.
    tf, tl, cf, cl = _separable()
    assert label_transfer(tf, tl, cf, cl)["transfer_dice"] > 0.9


def test_label_transfer_nan_when_target_empty():
    tf, tl, cf, cl = _separable()
    tl = torch.zeros_like(tl)                   # no target foreground
    assert label_transfer(tf, tl, cf, cl)["transfer_dice"] != \
           label_transfer(tf, tl, cf, cl)["transfer_dice"]   # nan
...
def test_prototype_cosine_soft_labels_dense():
    ...
    assert set(out) == {"auroc"}               # dense returns soft_auroc only
    assert out["auroc"] > 0.85
...
def test_prototype_cosine_separable_point():
    ...
    assert set(out) == {"auroc", "ap"}         # point returns hard auroc + AP
