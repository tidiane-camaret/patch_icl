"""Shape / contract / normalization checks for ResEncTSEncoder (encoder=resenc_ts)."""
import torch
...
def test_out_ch_and_forward_shape():
    enc = ResEncTSEncoder(resolution=16, n_stages=5, stages=(2, 3, 4),
    ...
    assert enc.input_norm == "passthrough"     # from-scratch default
...
def test_zscore_norm_path_runs():
    enc = ResEncTSEncoder(resolution=8, n_stages=4, stages=(1, 2, 3), input_norm="zscore",
                          frozen=False, device="cpu", precision="fp32")
