...
- multi-source dataloader: end-to-end smoke — `experiment=81_multisource_ct_mri` runs 1 epoch on the local RTX A6000 (tiny caps: max_ds_len_train=16, batch_size=2, eval_epoch_length=24, no OOM) through the full SpacingBatchSampler -> assemble_task -> CPU aug -> collate -> train step -> val loop -> build_sample_table path. Finite val dice 0.0102 (best), no traceback. Regime mix over 300 assemble_task calls: cross 93 / ct 99 / mri 108 — all three well represented. Ready for a full run.
- multi-source dataloader — final-review fix wave (F1-F7):
  - F1: `experiments/3d/eval.py`'s class-resolution ladder gains a `multisource` branch (`resolve_multisource_classes(cfg, "val")` + CT root); was falling through to `resolve_classes("union")` -> `ValueError: No classes found for split 'union'`.
  - F2: `build_dataset` multisource branch asserts each sub-provider has >=1 subject for >=1 class, else a pointed `ValueError` naming the source / modality / sub-split — catches a broken `split_map` or MRI spec that would silently degrade the run to pure-CT with bogus "cross" labels.
  - F3: dropped the `paths:` block from `dataset/multisource_ct_mri.yaml` — the dataset group composes AFTER `cluster` in `train.yaml`, so it overrode `cluster=meta` back to the NFS roots. Both roots already live in `cluster/{nfs,meta}.yaml`.
  - F4: `MultiSourceProvider` `meta` gains `fallback: bool` (a "cross" regime draw that collapsed to same-modality); `evaluate.py::_sample_detail` appends ` [fb]`.
  - F5: MRI eval class spec `per_source_val_classes` `[all, train]` -> `[all, all]` (the 50 `MRI_ALL_CLASSES`) so the cross-modality *generalization* eval has MRI unseen-class signal; MRI subject-split remap (`{val: test}`) unchanged.
  - F6: DRY `_multisource_specs(cfg, which)` helper (+ `source_mix` presence guard + sources/modalities/specs length-equality guard) shared by `resolve_multisource_classes` and `build_dataset`; `_check_multisource.py` prints "eval classes with no provider subjects"; assorted config-comment fixes (soft->occupancy remap is in `build_dataset`, ~117 eval classes, `eval.tasks_per_class` inert in cohort mode).
  - F7: regime-conditional class draw — the class is now drawn AFTER the regime from that regime's no-fallback pool (`_classes_by_mod` / `_both_classes`, precomputed in `__init__`), so the genuine cross-modality rate ~= `regime_p[cross]` (~1/3) instead of ~6.7% (a class-first uniform draw hit a CT-only class ~90% of the time). The per-slot modality fallback + `meta["fallback"]` are now a rare empty-pool safety net.
  - Spec `docs/superpowers/specs/2026-09-06-...` known-limitations updated: eval.py siblings (`probe_context_drift.py` / `feature_sim/probe_transformer.py` / `rawcheck_ab.py`) still lack a `multisource` branch; CT/MRI subject-id namespaces collide (`s0001…` in both roots) -> ~0.1% of cross tasks mis-flagged `self_ctx` (excluded from the val aggregate), left unfixed.
...
New `src/shapes3d/` package (`primitives.py`, `spec.py`, `instantiate.py`) generates procedurally-defined 3D shapes — not anatomically plausible by design — for a new `SynthGmmProvider` cohort mode, gated by a `p_shape` constructor kwarg (default `0.0`, off unless enabled). Today the only entry point is `SynthGmmProvider(..., p_shape=..., shape_spec=...)` directly — nothing threads `p_shape`/`shape_spec` through Hydra config or `experiments/3d/common.py` yet; that wiring is deferred/out of scope for this plan. When it fires, a shape is stamped into each cohort member's independently-sampled real gmm_bank host organ crop under a new pseudo-class id (`data/maisi_classes.py`, ids 195-198, `SHAPE_ID_TO_FAMILY`), loosely positioned inside the host (no reject-until-valid containment). Four independent `*_between_ratio` knobs (shape/size/position/intensity, `ShapeCohortSpec`) control how identical vs. independent each axis is across the K+1 cohort members — generalizing the cohort-shared-mu/sd-plus-per-member-between_ratio pattern `_draw_gmm`/`_build_nc` already used for intensity to shape, size, and position too. Cascade re-crop consistency reuses the existing `gmm_seed`/`member_idx`-keyed RNG scheme with one added subject-string field (the host's real class id, needed to re-resolve the shape's position at any re-crop level). Motivated by OOD eval (tumors/sub-regions) tending to fall back to known-organ shapes — see docs/superpowers/specs/2026-09-14-cohort-consistent-synthetic-shapes-design.md for the full design and the NV-Generate-CTMR/controlSynth/omniSynth prior art it builds on.

**Bug fix discovered during Task 4 implementation:** `rasterize_shape_in_crop` (src/shapes3d/instantiate.py) can raise `ValueError: assignment destination is read-only` when the crop array is a zero-copy view of the mmap_mode="r" bank file (e.g. when the resolved crop spans the full native mask array, `organ_crop_arrays` returns it unsliced, and `np.ascontiguousarray` returns a read-only view). Fixed in `_build_nc` (src/providers/synth_gmm.py) with a writeable-copy guard: `if not crop_lbl.flags.writeable: crop_lbl = crop_lbl.copy()` before stamping the shape. Test: `test_build_nc_handles_a_shape_stamped_into_a_full_array_readonly_crop` in src/providers/test_synth_gmm.py.

**Known limitation:** a shape's `size_frac` and `position_uvw` (src/shapes3d/instantiate.py) are defined relative to the *current* crop's own grid/bbox, not in world/mm space — unlike `mu`/`sd` (physically resolution-independent). Therefore, a shape's absolute physical size and position are NOT guaranteed consistent across cascade re-crop levels with different `crop_spacing_mm`. This limitation is safe for single-level evaluation only; future engineering wiring `p_shape > 0` into multi-level cascade training configs must account for this (either accept per-level shape redefinition, or pre-compute world-space shape coordinates — deferred, not yet solved).

Not yet wired into any training config or calibrated against eval — that's the deliberately deferred next step (see spec sec 7).

## 2026-09-14 — plot_shape_items.py: visualizing shape-mode cohorts

New `experiments/3d/plot_dataset_items.py` sibling, `experiments/3d/plot_shape_items.py`, for eyeballing the shape-mode cohorts above. Not built as a `plot_dataset_items.py` extension because that tool goes through Hydra's `build_dataset`, and `p_shape` has no config wiring (see above) — it's only reachable via `SynthGmmProvider`'s constructor kwargs. Instead this script constructs `SynthGmmMaisiDataset` + `SynthGmmProvider(cascade=True, p_shape=1.0, shape_spec=...)` directly against the real `gmm_bank`, calls `assemble_task` per row, and reuses `plot_dataset_items.py`'s `_best_slice`/`_overlay` helpers for rendering (imported, not duplicated). Flags: `--family {blob,splatter,disk,cylinder}` to force one family (default: mix), `--n_samples`/`--context_size` for grid size, and `--shape_between_ratio`/`--size_between_ratio`/`--position_between_ratio` to inspect the consistency knobs directly (e.g. all `0` → near-identical cohort, all `1` → fully independent members). Confirmed visually: cylinders render as elongated capsules (or a diagonal band when sliced near-edge-on), splatter as several scattered small blobs, blob/disk as round-to-oval regions — all stamped into real host organ crops with visible background variety across cohort members.

## 2026-09-15 — shape-mode world-space redesign: cascade-consistent shapes, wired into 92_multisource_synth

Fixed the Known Limitation from 2026-09-14: `size_frac`/`position_uvw` were resolved relative to the CURRENT crop's own grid/bbox, so a shape's absolute physical size and position drifted across cascade levels with different `crop_spacing_mm` — the documented reason `load_native_crop` raised `NotImplementedError` on any re-crop with a predicted center. Root cause: `mu`/`sd` are physically resolution-independent scalars, so the analogy to their cascade consistency didn't actually transfer to shape geometry.

**Redesign**: `ShapeCohortSpec` now draws `size_mm` (target diameter) and `position_offset_mm` (a 3-axis offset from the host's own precomputed centroid, `e["cents"]`) — both physical, level-invariant quantities — instead of crop-relative fractions. `SynthGmmProvider._build_nc` anchors the shape to `fallback` (the host centroid, unlike the resolved `center`, which can be a live `random_fg` draw or a cascade-predicted center and is NOT itself level-invariant), converts the fixed native-space center into the current crop's local array coordinates using `organ_crop_arrays`'s already-returned native origin (`crop_geom[0]`) and the cap-stride factor, and computes `mm_per_voxel` fresh each call from the host's own native spacing times that stride. `src/shapes3d/primitives.py`'s API changed from crop-relative fractions (`size_frac`, `length_frac`, `spread_frac`) to absolute local-voxel quantities (`size_vox`, `length_vox`, `spread_vox`) resolved by the caller (`rasterize_shape_in_crop`) from the physical mm values — this is what makes cross-resolution consistency exact rather than approximate, and it also decouples primitive sizing from the array shape entirely.

**Bonus**: decoupling sizing from the array shape enabled a real fix for the previously-flagged rasterization-cost problem (Important #3, final review 2026-09-14) for free — `rasterize_shape_in_crop` now rasterizes into a small local sub-box (sized by the new `primitives.reach_vox`, a generous per-family bounding-radius estimate) instead of the full crop, then copies the result in at the right offset. Measured: splatter at a 256³ crop, worst-case params, dropped from the reviewer's reported ~10.5s to ~1.4s (7.7×) — and that's the *worst case* (1mm/voxel); realistic wide-FOV level-0 crops have much coarser `mm_per_voxel`, giving proportionally smaller sub-boxes and bigger wins. This also removed the `np.argwhere(crop_lbl == host_cls_id)` bbox scan the old crop-relative position logic needed (position no longer depends on scanning the crop at all).

Verified end-to-end against the real `gmm_bank` (not just the tiny test fixtures): a 30mm-diameter blob rasterized at a coarse level-0 crop (6mm/voxel, 23760mm³ realized — heavily quantized, only ~5 voxels across) and re-derived at a narrow simulated level-1 crop (1.5mm/voxel, with the host's true centroid standing in for a perfect predicted center) came out at 17266mm³ — much closer to the true 14137mm³ target than level-0's estimate, exactly as expected from better discretization at finer resolution, not a different object.

Removed the `NotImplementedError` guard in `load_native_crop` (no longer needed — a predicted `center` only affects the crop WINDOW, not the shape's own anchor) and the two tests that asserted it. `ShapeCohortSpec`'s `flatten_axis` (disk's one discrete shape param) also now respects `shape_between_ratio` via a probabilistic redraw (`shape_between_ratio=1.0` → always redraws per member) — it previously never redrew regardless of the ratio, a real per-axis-consistency leak (final review Important #4), fixed alongside since this pass already touched the relevant code.

**Wired into config**: `experiments/3d/common.py`'s multisource+synth path now reads `data.gmm.p_shape`/`data.gmm.shape` and threads them into `SynthGmmProvider`. `configs/experiment/3d/experiment/92_multisource_synth.yaml` sets `p_shape: 0.1` (conservative starting point, not calibrated against eval) with `shape: {}` (`ShapeCohortSpec` defaults). Config compose verified directly (`hydra.compose` + `ShapeCohortSpec(**kwargs)` construction).

Still open: neither the `size_mm`/`spread_mm`/`position_offset_mm` default ranges nor `p_shape=0.1` are calibrated against eval — this is a starting point, not a tuned value.

**`plot_shape_items.py --cascade_spacings`**: new mode visualizing the fix directly -- columns become cascade levels (e.g. `6,3,1.2`) instead of target/context, showing the TARGET member re-cropped at each spacing. Level 0 comes from `assemble_task`; each later level re-derives the same subject via `load_native_crop` with the host's own true centroid as an explicit center (a perfect-predictor stand-in, since there's no real model in this tool). Confirmed visually across blob/splatter/cylinder: the shape stays in the same anatomical location and gets progressively sharper across 6→3→1.2mm, rather than jumping to a different object per level.

## 2026-09-15 — synth_gmm cascade path now uses GPU realize (was CPU-only)

`data.gpu_realize` (the `SynthGmmMaisiDataset`/`SynthRealizer` mechanism) only ever applied to the standalone non-cascade `data.source=synth_gmm_maisi` path — `common.py` hardcodes `gpu_realize=False` for the multisource+`p_synth` path (`92_multisource_synth.yaml` and friends), with the comment "cascade path builds NativeCrop directly." That cascade path's `_build_nc` did the GMM paint *and* the antialiased resample-to-grid itself, on CPU, then shipped an already-fully-realized `T³` crop. Meanwhile `data.gpu_realize_crop` — a separate, already-on-by-default mechanism for the cascade path (`src/gpu_realize_crop.py`, generic over any provider's `NativeCrop`) — was already running on every synth/shape crop, but as a pure no-op pass-through, since there was nothing left for it to resample.

Fixed by splitting the two steps that `_build_nc` used to do together: paint (cheap — `mu[crop]+sd[crop]*noise`, a vectorized numpy op) stays on CPU at native (post-cap) resolution; the resample-to-grid (the actually expensive part) is no longer done here at all — `_build_nc` now ships the native painted crop directly, with real `out_sizes`/`pad_lo` from `organ_crop_arrays` (not the old hardcoded `[T,T,T]`/`[0,0,0]`) and `decim=step` (the existing `gpu_realize_max_native` cap-stride, reused as the shipped array's already-applied decimation factor). The already-active `gpu_realize_crop` step now does real work for synth/shape cohorts too, batched on GPU with every other source in the step.
