"""
Adapter for the NNInteractive experiment (experiments/nninteractive/).

Mirrors MultilevelICLAdapter but drives inference with NNInteractiveEncoder:
  - context images are encoded WITH their ground-truth masks (mask-conditioned features)
  - target images are encoded with a zero mask

Checkpoint format (saved by experiments/nninteractive/train.py):
...
from experiments.nninteractive.train import encode_target, encode_context
from experiments.multilevel.model import MultilevelICL
from src.models.encoders.nninteractive import NNInteractiveEncoder


class NNInteractiveAdapter(InContextModel):
    """
    Wraps the NNInteractive-encoder + MultilevelICL stack for the quality benchmark.

    Inference mirrors experiments/nninteractive/train.py:validate().
    The final grid prediction is upsampled trilinearly to the input spatial size.

    scale_mm is estimated from the stored image_size and a default 1.5 mm/voxel
    isotropic spacing (TotalSegmentator standard).  Pass spacing_mm to override.
    """
    ...
    def __init__(
        ...
        nnint_ckpt: str = None,          # override the encoder checkpoint path stored in config
        spacing_mm: float = 1.5,         # assumed isotropic voxel spacing (mm) for scale_mm
    ...
    def predict(self, target_img, context_imgs, context_masks):
        ...
        for i, res in enumerate(self.resolutions):
            ...
            # scale_mm: effective mm per patch at this level
            scale_mm = torch.full(
                (B,), (self.image_size[0] / res[0]) * self.spacing_mm,
                device=self.device,
            )
        ...
        # Upsample final grid to input spatial size
