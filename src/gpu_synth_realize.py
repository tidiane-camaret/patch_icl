...
@torch.no_grad()
def _resample_member(native, out_size, T, pad_lo, cls, occ_thr, mask_downsample):
    """Native-res multiclass crop (D',H',W' long, on device) -> placed T^3 (paint_lab, mask).

    Decoupled by role (GPU port of SynthGmmMaisiDataset._resample_paint_mask + place_label):
      paint_lab: full multiclass, NEAREST — only drives the per-voxel GMM shade, so it is
                 treated like an IMAGE (no enlargement).
      mask:      target-class BINARY under mask_downsample. "occupancy" area-pools the target
                 ...
                 matches totalseg resample_binary); "nearest" = paint==cls. Non-empty guard."""
    device = native.device
    size = tuple(int(s) for s in out_size)
    paint = F.interpolate(native.float()[None, None], size=size, mode="nearest")[0, 0].long()
    ...
    if all(s == T for s in size):
        return paint, mask
    fp = torch.zeros(T, T, T, dtype=torch.long, device=device)
    ...
    def __call__(self, batch, device):
        ...
        for b in range(B):
            ...
            for t in range(Kp1):
                ...
                paint, mask = _resample_member(native, out_sizes[b, t], T, pad_lo[b, t],
                                               cb, self.occ_thr, self.mask_downsample)
                noise = torch.randn(paint.shape, generator=gen, device=device)
                img = (mu[b][paint] + sd[b][paint] * noise - GMM_MEAN) / GMM_STD   # == _paint
                mimg.append(img[None])                                 # (1,T,T,T)
